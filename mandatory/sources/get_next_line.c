...
/*   Updated: 2022/07/12 02:37:25 by gamoreno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/* Parametros: -Un entero (file descriptor) 'fd'.
 *
 * Esta funcion toma el archivo referenciado por el file descriptor 'fd' y sin
 * importar el BUFFER_SIZE dado lee e imprime en pantalla la primera linea del
 * archivo ayudado de la funcion read, asi, cada vez que se invoque la funcion
 * devolvera la linea siguiente hasta terminar el archivo.
 *
...
char	*get_next_line(int fd)
{
	...
	if (!static_buffer)
		return (NULL);
	line = do_line(static_buffer);
	static_buffer = fix_buffer(static_buffer);
	return (line);
}
